<div>
     <div class="d-flex justify-content-between">
          <h3 class='p-2'>
               {{ __('translation.realstates') }}
          </h3>
          <div class="btn-group">
               <div class="" style='display:inline ;margin:3px'>
                    <a href="{{ route('realstate.create') }}" class="btn btn-primary btn-sm">
                         {{ __('translation.add_realstate') }}
                    </a>
               </div>
          </div>
     </div>
     <div class="table-responsive">
          <table class="table align-middle table-row-dashed fs-6 gy-5" id="">
               ...
               <tr>
                    <th class="">{{ $item->id }}</th>
                    <th class="">{{ $item->realstate_number }}</th>
                    <th class="">{{ $item->national_address  ?? ''}}</th>
                    <th class="">{{ $item->instrument_number }}</th>
                    <th class="">{{ $item->Province->name ?? '' }}</th>
                    <th class="">{{ $item->SubArea->name }}</th>
                    <th class="">{{ $item->opration_precentage ?? '-' }}</th>
                    <th class="">{!! $item->getStatusWithSpan() !!}</th>
                    <th class="">{{ $item->created_at->format('Y-M-D') }}</th>
                    <th class="">
