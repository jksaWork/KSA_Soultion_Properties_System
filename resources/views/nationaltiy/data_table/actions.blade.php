...
<div style="min-width: 200px">
     ...
     <a type="button" data-bs-toggle="modal" data-bs-target="#edit_bank_{{ $bank->id }}" href="#" class="btn btn-light-info btn-sm btn-icon me-1">
     ...
     <div class="modal fade" id="edit_bank_{{ $bank->id }}" tabindex="-1" aria-hidden="true">
          ...
          <div class="modal-dialog modal-dialog-centered mw-650px">
               ...
               <div class="modal-content">
                    <!--begin::Form-->
                    <form class="form" action="{{ route('nationnalty.update' , $bank->id) }}" method="post">
                         @method('PUT')
                         @csrf
                         <!--begin::Modal header-->
                         ...
                         <div class="modal-body py-10 px-lg-17">
                              ...
                              <div class="scroll-y me-n7 pe-7" id="#updatemodel">
                                   <x:text-input class="col-md-12" name='name' value="{{ $bank->name }}" />
                              </div>
     ...
     <a href="#" onclick="SwitchStatus('{{ route('nationnalty.show' , ['nationnalty' => $bank->id, 'status' => 1])}}')" class="btn btn-light-success btn-sm btn-icon me-1">
          <span class="svg-icon svg-icon-3">
               <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
     ...
     {{-- <button onclick="SwitchStatus({{ $id }})alertWithId({{ $id }})">asdasdas</button> --}}

     {{-- @endif --}}

     {{-- @if (auth()->user()->hasPermission('delete_admins')) --}}
     {{-- <form action="{{ route('banks.destroy', $id) }}" class="my-1 my-xl-0" method="post" style="display: inline-block;">
     @csrf
     @method('delete')
